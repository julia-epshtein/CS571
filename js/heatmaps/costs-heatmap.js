const costsHeatmapConfig = {
  width: 900,
  height: 600,
  margin: { top: 70, right: 30, bottom: 30, left: 150 },
  colorScheme: d3.interpolateYlOrRd, 
  title: "Imprisonment Costs (Millions)",
  containerId: "#costs-heatmap-container",
  visibleCounties: 10,
};
...
function processHeatmapData(data, countyCol, yearCol, valueCol) {
  ...
  // Get counties and sort A-Z
  const counties = Array.from(grouped.keys()).sort((a, b) =>
    a.localeCompare(b)
  );

  // Get years and sort ascending
  const years = Array.from(new Set(data.map((d) => d[yearCol]))).sort(
    d3.ascending
  );
...
function createHeatmap(heatmapData, config) {
  const container = d3.select(config.containerId).html("");

  // Title
  container.append("div").attr("class", "heatmap-title").text(config.title);
  ...
  const colorScale = d3
    .scaleSequential(config.colorScheme)
    .domain([0, maxValue]);
  ...
  svg
    .selectAll()
    .data(
      heatmapData.matrix.flatMap((row, i) =>
        row.map((value, j) => ({
  ...
  const legend = legendSvg.append("g").attr("transform", "translate(0, 20)");
  ...
  // Update the SVG height to accommodate the total counties
